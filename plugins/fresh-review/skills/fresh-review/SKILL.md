...
This is one of two places the skill stops for an answer; the other is a re-review whose delta needs the user's choice (Step 1.6). It never assumes approval: when no one can answer (AskUserQuestion is unavailable or fails), the run stops at the gate, restores the tree, and says how to continue. Pass `--arch-approved` to skip the gate — for a re-run after approving, or when the user says the architecture is already agreed ("architecture approved", "skip the architecture gate"). Resolve it once, from the invocation, like `--codex`. On a re-review, an approval from the last review carries over: the gate asks again only about new decisions (Step 4.8a).
...
An architecture the user approved at the last review stays approved. Pass D still checks the change, but it only asks about decisions the user has not seen.
...
When a prior review exists, it also reports `PRIOR_COVERAGE` (`full` or `reduced`) and `PRIOR_ARCH_GATE` (`approved` when the user approved the architecture at that review, or at a review it carried the approval from). With `PRIOR_ARCH_GATE: approved`, it copies the approved decisions to `$RUN_DIR/delta/prior-architecture.md` for Pass D, when they are on disk.
...
>
> When `{{RUN_DIR}}/delta/prior-architecture.md` exists, read it — the only file under `{{RUN_DIR}}/delta/` you may read. It holds the architecture decisions an earlier isolated pass found in this PR, from the code alone. Mark each decision you name `seen: <the earlier decision>` when it is the same structural choice (this change extends or adjusts it, and its shape — components, edges, contract, owner — stays the same), and `new` otherwise. When unsure, mark it `new`.
>
...
> ---
> DECISIONS
> - D1: <the decision in one sentence> | evidence: <file>[, <file>] | new | seen: <earlier decision>
> ---
...
Audit Pass D's `FILES_READ` against the forbidden list now, by the Step 6 rules. Pass D may read `delta/prior-architecture.md`; any other `delta/` path is a leak. A Pass D that read a plan or `pr-context/` describes the author's claim, not the code. On a leak, re-spawn it once; on a second leak, prefix the gate block `[intent-contaminated]`.
...
- `ARCH_DECISIONS: yes`, `DELTA_PRIOR_ARCH_GATE: approved`, and every decision is marked `seen` → the user approved these at the last review. Record `ARCH_GATE='carried'` in `state.env`, print `Architecture: approved at the last review (<DELTA_PRIOR_RUN or PRIOR_REVIEW>) — no new design decisions. Reviewing the implementation.`, then one line per decision, `D<k> <decision> → seen: <earlier decision>`, so the reader can check each match without being asked. Go to Step 5. Do not run Pass E and do not ask.
- `ARCH_DECISIONS: yes` with at least one `new` decision, after an approved last review → continue to 4.8b, but list and ask about the `new` decisions only. Under the diagram, say `Approved at the last review: D<k>, …` for the `seen` ones.
...
```json
{"skill":"fresh-review","schema":12,"run_id":"<RUN_ID>","plugin_version":"<FR_VERSION>",
 ...
 "architecture":{"gate":"<not_needed|approved|carried|rejected|pending|skipped|failed>","decisions":0,"diagram":false},
...
- **Set `architecture` from Step 4.8.** `gate` is `skipped` under `--arch-approved`, `failed` when Pass D failed, `not_needed` when Pass D found no decisions, `carried` when every decision was approved at the last review, and otherwise the `ARCH_GATE` answer. `decisions` is Pass D's count; `diagram` is whether one was shown. Omit the `architecture` pass when the gate was skipped, and the `eng-review` pass whenever Pass E did not launch (no decisions, gate skipped, or `HAS_GSTACK: 0`). When the gate did not clear, the implementation passes never launched — omit them too, and set `verdict` to `ARCH-PENDING` for a pending gate.
...
- **The architecture was approved at the last review** (`PRIOR_ARCH_GATE: approved`) → Pass D still runs. When all its decisions are ones the user approved, the gate is `carried` and the run does not ask again (Step 4.8a). It asks only about new decisions.
